...
const loadShader = async (url) => {
    ...
    if (!response.ok) {
        throw new Error(`Error al cargar el shader desde ${url}`);
    }
    return await response.text();
}

// Vertex
const halftoneVertexShader = await loadShader('/static/shaders/halftone/vertex.glsl')
// Fragment
const halftoneFragmentShader = await loadShader('/static/shaders/halftone/fragment.glsl')
...
materialParameters.color = '#f09800'
materialParameters.shadowColor = '#b81949'
materialParameters.lightColor = '#e5ffe0'
...
const material = new THREE.ShaderMaterial({
    ...
    uniforms:
    {
        ...
        uShadowRepetitions: new THREE.Uniform(140),
        uShadowColor: new THREE.Uniform(new THREE.Color(materialParameters.shadowColor)),
        uLightRepetitions: new THREE.Uniform(175),
